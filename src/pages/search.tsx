        ...
        <aside className="bg-white rounded-2xl border shadow-sm p-5 h-fit">
          <form method="GET" className="space-y-4">
            ...
            <div className="flex gap-3">
              ...
              <Link className="rounded-lg px-4 py-2 border" href="/search">
                Limpiar
              </Link>
            </div>
          </form>
...
export const getServerSideProps: GetServerSideProps<SearchProps> = async (ctx) => {
  ...
  const loc     = String(ctx.query.location ?? "").trim();
  const minStr  = String(ctx.query.min ?? "").trim();
  const maxStr  = String(ctx.query.max ?? "").trim();
  ...
  if (q) {
    // SQLite no soporta 'mode: "insensitive"'. Usamos 'contains' a secas.
    // (SQLite ya compara case-insensitive en ASCII por defecto)
    const ors: any[] = [
      { title:    { contains: q } },
      { location: { contains: q } },
    ];
    // Intentar "description" si existe en el schema
    try { ors.push({ description: { contains: q } }); } catch {}
    where.OR = ors;
  }

  if (loc) where.location = { equals: loc };

  if (min !== null || max !== null) {
    where.priceUsd = {};
    if (min !== null && !Number.isNaN(min)) where.priceUsd.gte = min;
    if (max !== null && !Number.isNaN(max)) where.priceUsd.lte = max;
  }
